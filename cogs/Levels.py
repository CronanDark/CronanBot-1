...
class Levels:
    ...
    @_rank.command(pass_context=True, no_pm=True)
    @checks.admin_or_permissions(manage_server=True)	
    async def leaveo (self, ctx, user : discord.Member):
    ...
    async def creators(self, ctx):
        ...
        user = ctx.message.author
        await self.bot.say("```There were hard efforts in the making of this cog.\n \n It took LOTS of time to make a ranking system for Cronan, that could replace MEE6's ranking system.```")
