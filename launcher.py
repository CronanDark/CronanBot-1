...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="CronanBot - Discord Bot's launcher")
...
def update_red():
    try:
        ...
        return
    if code == 0:
        print("\nCronan has been updated")
    else:
        print("\nCronan could not update properly. If this is caused by edits "
...
def reset_red(reqs=False, data=False, cogs=False, git_reset=False):
    ...
    if git_reset:
        ...
        if code == 0:
            print("Cronan has been restored to the last local commit.")
        else:
...
def download_ffmpeg(bitness):
    clear_screen()
    repo = "https://github.com/CronanDark/CronanBot/raw/master/"
    verified = []
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Cronan:")
        print("1. Update Cronan + requirements (recommended)")
        print("2. Update Cronan")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Cronan (discards code changes, keeps data intact)")
...
def run_red(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Cronan. "
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Cronan...")
                continue
            else:
                if not autorestart:
                    break

    print("Cronan has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Cronan without going
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Cronan Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Cronan has been "
        ...
        print("1. Run Cronan /w autorestart in case of issues")
        print("2. Run Cronan")
        print("3. Update")
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Cronan needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Cronan cannot work without the pip module. Please make sure to "
